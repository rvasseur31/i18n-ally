...
export class CurrentFileExtractionItem extends BaseTreeItem {
  ...
  get description() {
    if (this.collapsibleState === TreeItemCollapsibleState.None) {
      ...
      else return length == null ? '' : length.toString()
    }
  }

  // @ts-expect-error
  get iconPath() {
    if (this.collapsibleState === TreeItemCollapsibleState.None) return this.getIcon('symbol-string-gray')
    else return this.getIcon('symbol-string')
  }

  async getChildren() {
    if (this.collapsibleState === TreeItemCollapsibleState.None) return []
