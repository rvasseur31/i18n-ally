...
export async function FulfillAllMissingKeys(prompt = true, extraKeys?: string[]) {
  if (prompt) {
    const Yes = i18n.t('prompt.button_yes')
    const result = await window.showWarningMessage(i18n.t('prompt.fullfill_missing_all_confirm'), { modal: true }, Yes)
    if (result !== Yes) return
  }
...
export async function FulfillKeys(item?: LocaleTreeItem | ProgressSubmenuItem | CommandOptions) {
  ...
  if (!item) pendings = await FulfillAllMissingKeys()

  if (item instanceof ProgressSubmenuItem) pendings = await FulfillMissingKeysForProgress(item)

  if (pendings?.length) await CurrentFile.loader.write(pendings, false)
}
