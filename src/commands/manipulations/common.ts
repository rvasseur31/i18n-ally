...
export function getNodeOrRecord(item?: LocaleTreeItem | CommandOptions): LocaleNode | LocaleRecord | undefined {
  ...
  if (item instanceof LocaleTreeItem) {
    return item.node.type !== 'tree' ? item.node : undefined
  }

  if (item.locale) return CurrentFile.loader.getRecordByKey(item.keypath, item.locale, true)
  else return CurrentFile.loader.getNodeByKey(item.keypath, true)
}

export function getNode(item?: LocaleTreeItem | CommandOptions | ProgressSubmenuItem) {
  if (!item) return

  if (item instanceof ProgressSubmenuItem) return
...
export async function getRecordFromNode(node: Node, defaultLocale?: string) {
  ...
  if (node.type === 'record') return node

  if (node.type === 'node') {
    const locales = CurrentFile.loader.getShadowLocales(node)
    const locale =
      defaultLocale ||
      (await window.showQuickPick(Global.visibleLocales, { placeHolder: i18n.t('prompt.choice_locale') }))
    if (!locale) return
