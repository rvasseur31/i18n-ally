...
export function detect(
  ...
  const { attributes: ATTRS, ignoredTags: IGNORED_TAGS, vBind: V_BIND } = Object.assign({}, defaultOptions, userOptions)
  ...
  const parser = new Parser(
    {
      onopentag(name, attrs) {
        lastTag = name
        if (name === 'script') lastScriptIndex = parser.endIndex! + 1
        ...
        const attrNames = Object.keys(attrs)
          .map(name => {
            // static
            if (ATTRS.includes(name) && shouldExtract(attrs[name], rules)) return [name, false]
            // dynamic
            else if (
              V_BIND &&
        ...
        for (const [name, isDynamic] of attrNames) {
          const match = code.match(new RegExp(`\\s${name}=(["'])([^\\1]*?)\\1`, 'm'))
          if (!match) continue
      ...
      onclosetag(name) {
        if (name !== 'script' || lastScriptIndex == null) return
        const start = lastScriptIndex
        const fullText = input.slice(start, parser.startIndex!)
        if (extractScripts) detections.push(...shiftDetectionPosition(extractScripts(fullText, start), start))
        lastScriptIndex = null
      },
      ontext(fullText) {
        if (lastScriptIndex != null) return
        if (IGNORED_TAGS.includes(lastTag)) return
        ...
        if (!shouldExtract(text, rules)) return
