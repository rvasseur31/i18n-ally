...
export class ComposedLoader extends Loader {
  constructor() {
    super('[Composed]')
    this._disposables.push(Translator.onDidChange(() => this._onDidChange.fire('')))
  }
  ...
  get root(): LocaleTree {
    ...
    for (const loader of this._loaders) {
      ...
      for (const key of Object.keys(loaderChildren)) children[key] = loaderChildren[key]
    }
  ...
  get flattenLocaleTree(): FlattenLocaleTree {
    ...
    for (const loader of this._loaders) {
      ...
      if (loaderChildren) Object.assign(this._flattenLocaleTree, loaderChildren)
    }
  ...
  async write(pendings: PendingWrite | PendingWrite[], triggerFullfilled = true) {
    ...
    if (Config.keepFulfilled && triggerFullfilled) {
      pendings = [...((await FulfillAllMissingKeys(false, uniq(pendings.map(i => i.keypath)))) || []), ...pendings]
    }

    pendings = pendings.filter(i => i)

    const distrubtedPendings: PendingWrite[][] = Array.from({ length: this.loadersReversed.length })
      .fill(null)
      .map(() => [])
    ...
    await Promise.all(
      loaders.map(async (loader, index) => {
        if (distrubtedPendings[index] && distrubtedPendings[index].length) await loader.write(distrubtedPendings[index])
      }),
    )
  }
