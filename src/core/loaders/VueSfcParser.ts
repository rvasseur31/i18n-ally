        ...
        if (currentBlock.src) {
          try {
            ...
            if (lang === 'yaml' || lang === 'yml')
              messages = load(fileContent) || {}
            else if (lang === 'json5')
              messages = JSON5.parse(fileContent)
            else
          ...
          try {
            ...
            if (currentBlock.lang === 'yaml' || currentBlock.lang === 'yml')
              messages = load(content) || {}
            else if (currentBlock.lang === 'json5')
              messages = JSON5.parse(content)
            else
  ...
  for (const block of sortedBlocks) {
    if (block.src) {
      ...
      try {
        ...
        let serialized = ''
        if (lang === 'yaml' || lang === 'yml')
          serialized = dump(block.messages, { indent: 2 })
        else if (lang === 'json5')
          serialized = JSON5.stringify(block.messages, null, 2)
        else
          serialized = JSON.stringify(block.messages, null, 2)
